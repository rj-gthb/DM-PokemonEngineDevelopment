    ...
    const generateTeam = async () => {
        ...
        try {
            ...
            if (available.length === 0) {
                throw new Error('No available Pokemon found for the selected region.');
            }

            // Collect a randomized pool of candidate details, but tolerate intermittent fetch failures.
            const candidatePoolIds = available
                .sort(() => 0.5 - Math.random())
                .slice(0, Math.min(30, available.length))
                .map(p => p.id);

            const candidateResults = await Promise.allSettled(candidatePoolIds.map(id => fetchDetails(id)));
            const candidatePool = candidateResults
                .filter(result => result.status === 'fulfilled' && result.value)
                .map(result => result.value);

            if (candidatePool.length < 6) {
                throw new Error('Unable to load enough Pokemon details to generate a team. Please try again.');
            }
            ...
            console.error(error);
            alert(error?.message || 'Failed to generate challenger team');
        } finally {
            setGenerating(false);
        }
    };
